...
void SplatmapMix(float3 positionWS, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    Light mainLight = GetMainLight();
    
    half4 diffAlbedo[4];
    diffAlbedo[0] = (SAMPLE_TEXTURE2D(_Splat0, sampler_Splat0, uvSplat01.xy) + 0.6) * (half4(GrassColorWS(0.036, positionWS), 1) * 0.9h * half4(mainLight.color, 1));
...
#endif
{
    ...
    SplatmapMix(IN.positionWS, IN.uvSplat01, IN.uvSplat23, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS);
